...
def main():
    ...
    ap.add_argument("--out_dir", default=os.path.join("out", "lab4"))
    ...
    for _, fp in iter_text_files(args.input_dir):
        ...
        for t in toks:
            s = simple_stem_en(t)
            stems.append(s)
            if len(examples) < args.examples_n and t != s and t not in examples:
                examples[t] = s

        total_tokens += len(stems)
        total_token_chars += sum(len(t) for t in stems)
        term_freq.update(stems)
    ...
    stats = []
    stats.append("=== Lab04 Stemming statistics ===")
    ...
    stats.append("")
    stats.append("Examples (token -> stem):")
    for k, v in list(examples.items())[: args.examples_n]:
        stats.append(f"  {k} -> {v}")
    stats_txt = "\n".join(stats) + "\n"
